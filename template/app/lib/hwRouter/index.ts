...
const hwRouterHelper = new HwRouterHelper()
 ...
 * @param prefix 前缀
 */
function HwController(prefix: string) {
  ...
  return (target: any) => {
    // 获取class名
    // todo：比较hack，待确认是否有更好的方法获取class名
    // const key = target.toString().split(' ')[1];
    const key = target.getName()
    if (!hwRouterHelper.controllers[key]) {
      hwRouterHelper.controllers[key] = {
        target,
        prefix,
        routers: []
      }
    } else {
      hwRouterHelper.controllers[key].target = target
      hwRouterHelper.controllers[key].prefix = prefix
    }
  }
}
...
function request(path: string, method: RequestMethods) {
  return function (target: any, value: any, des: PropertyDescriptor & ThisType<any>) {
    ...
    hwRouterHelper.controllers[key].routers.push({
      handler: value,
      path,
      method
    })
...
function Time(format: string = '') {
  return (value: any, key: string) => {
    ...
    if (!format) {
      return dayjs(value).utcOffset(480)
    }
    return dayjs(value).utcOffset(480).format(format)
  }
}
...
/**
 * 抛出hwrouter，在router.ts中直接使用HwRouter(app);即可完成自动注入路由
 * @param app application
 * @param options 参数，目前只有prefix，就是所有路由的前缀
 */
export function HwRouter(app: Application, options?: { prefix?: string }) {
  const { router } = app
  if (options && options.prefix) {
    router.prefix(options.prefix)
  }
  hwRouterHelper.injectRouter(router)
}
